...
내가 이 논문에서 얻은 가장 큰 교훈은 다음이다.

> AI가 생성한 결과도 결국 외부 입력이다.  
> 따라서 보안 관점에서는 LLM output 역시 검증해야 한다.

이 관점은 앞으로 내가 보안 학습 도우미나 RAG 기반 프로젝트를 만들 때 중요한 기준이 될 것 같다. 단순히 답변을 잘하는 시스템이 아니라, 근거를 제시하고, 위험한 출력을 줄이고, 사용자가 신뢰할 수 있는 방식으로 정보를 제공하는 시스템을 만드는 것이 필요하다.
