...
2.3 방대한 데이터와 비효율성
보안 문서는 방대한 양을 가지며 지속적으로 업데이트된다. 이를 수동으로 탐색하고 비교하는 과정은 비효율적이며 학습 부담을 증가시킨다.
...
5. 개발 과정
본 프로젝트는 오픈소스인 Penetration Testing RAG를 기반으로 진행되었으며, 다음과 같은 단계로 개발이 이루어졌다.
...
6. 구현 결과 (영상)
다음 영상( https://youtu.be/dPnZSiQyD8o )은 본 시스템의 실제 구현 및 동작 과정을 보여준다.
